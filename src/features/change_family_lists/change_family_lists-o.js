...
function newPersonRecord() {
  return {
    // Standard WikiTree-style fields
    Name: "",
    FullName: "",
    BirthDate: "",
    DeathDate: "",
    ...
    // Single string marriage info (e.g. "— married 31 Mar 1834 (to 1855) in Geauga County...")
    MarriageDetails: "",

    // If you still want the map link separate, keep this:
    MarriageMapLink: "",
  };
}
...
/************************************************
 * 2) PARSING ITEMPROP ELEMENTS (PARENTS, SIBLINGS, CHILDREN)
 ************************************************/
function parseItempropElement(el) {
  const record = newPersonRecord();

  // If there's a link to a wiki profile
  ...
  // If there's a <span class="SMALL">(1930s - unknown)</span>
  ...
  // If it's a bracketed private name, e.g. "[private father (1930s - unknown)]"
  ...
  if (!record.FullName) {
    ...
    if (bracketMatch) {
      ...
      // If there's a date in parentheses
      ...
      // Possibly just a plain text name
...
/************************************************
 * 3) PARSING LEFTOVER BRACKETED TEXT
 *    (e.g. "[children unknown]" or "[brothers or sisters?]")
 ************************************************/
...
function parseBracketedUnknownInBlock(blockEl) {
  ...
  // 2) bracketed text not in anchors
...
/************************************************
 * 4) PARSE BLOCK (PARENTS, SIBLINGS, CHILDREN)
 ************************************************/
...
/************************************************
 * 5) SPECIAL PARSER FOR SPOUSES BLOCK (with marriage details)
 ************************************************/
/**
 * parseSpousesBlock extracts a spouse's name/link from <span itemprop="spouse">,
 * then captures the entire marriage line (e.g. "— married 31 Mar 1834 (to 1855) in Geauga County, Ohio, United States")
 * as a single string in record.MarriageDetails.
 */
function parseSpousesBlock(spousesEl) {
  const html = spousesEl.innerHTML;
  // We split by "Husband of", "Wife of", or "Spouse of"
  ...
  chunks.forEach((chunk, i) => {
    if (i === 0) {
      // This is text before the first "Husband of" or "Wife of"
      return;
    }
    ...
    if (spouseSpan) {
      // merge the spouse data into rec
      const spouseData = parseItempropElement(spouseSpan);
      Object.assign(rec, spouseData);
    }

    // Step B: capture the entire "— married ..." line as a single string
    // We'll look for "— married" in the text
    const chunkText = div.innerText;
    const idx = chunkText.toLowerCase().indexOf("— married");
    if (idx >= 0) {
      // Store everything from "— married" to the end
      const details = chunkText.slice(idx).trim();
      rec.MarriageDetails = details;
    }

    // Step C: if there's a map link, store it separately if desired
    const mapLink = div.querySelector('a[href*="maps.google"]');
    if (mapLink) {
      rec.MarriageMapLink = mapLink.getAttribute("href");
    }

    // Only add the record if it has something
...
function parseInitialData() {
  ...
  if (spousesBlock) {
    // Parse spouse entries with marriage info
    const spouseEntries = parseSpousesBlock(spousesBlock);
    familyData.spouses.push(...spouseEntries);

    // Also parse leftover bracket text in #Spouses,
    // e.g. "[spouse(s) unknown]"
