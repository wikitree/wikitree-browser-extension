...
export function createChatAiPlannerHandlers({
  ...
  async function tryAiExpandConnectionTarget(target, prompt) {
    ...
    const aiPrompt = [
      ...
      "Include a likely birth year when it helps disambiguate the person.",
      "Return a JSON object with one of these shapes:",
      '{"searchName":"<canonical full name>","birthYear":1809} OR {"searchName":"<alternate search name>"} OR {"wtId":"Name-123"} OR {"none":true}',
      "Only return valid JSON (no markdown).",
      "Examples:",
      '- Target: "Darwin" with prompt about a famous naturalist -> {"searchName":"Charles Darwin","birthYear":1809}',
      '- Target: "JFK" -> {"searchName":"John Fitzgerald Kennedy","birthYear":1917}',
      `Target: ${target}`,
      `Prompt: ${prompt}`,
    ].join("\n\n");
