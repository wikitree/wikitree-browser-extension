...
import { secondarySort } from "../wills/wills";
...
async function initSurnameTableSorting() {
  ...
  if (theTable.length) {
    ...
    let birthHeader = headerRow.find("th").eq(1);
    ...
    if (deathHeader) {
      ...
      const editDateHeader = headerRow.find("th").eq(6);
      editDateHeader.attr("id", "editDateHeader");

      const PMHeader = headerRow.find("th").eq(5);
      PMHeader.attr("id", "PMHeader");

      const nameHeader = headerRow.find("th").eq(0);
    ...
    $("#birthLocationWord").on("click", function (e) {
      ...
      const rows = theTable.find("tbody tr:not(.filter-row)");
      rows.slice(0, 1);
      ...
      theRows.removeClass("firstBirthLocation");
      theTable.find("tr[data-birth-location-small2big!='']").eq(0).addClass("firstBirthLocation");
    });
  }
...
async function initSurnameTableSorting() {
  ...
  if (isSearchPage) {
    ...
    $("#managerWord").on("click", function () {
      ...
      if (theseRows.length) {
        ...
        theseRows.appendTo($("table.wt.names"));
        dNumbering();
  ...
  theTable.find("tr").each(function () {
    ...
    if (isSpecialWatchedList) {
      ...
      const combinedText = combinedTD.text().trim() || "";
...
async function addWideTableButton() {
  ...
  wideTableButton.on("click", function (e) {
    ...
    if (isSpecialWatchedList) {
      theTable = $("body.watchedList table.wt.table");
    }
