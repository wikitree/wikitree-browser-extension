...
chrome.storage.sync.get('locationsHelper', (result) => {
	if (result.locationsHelper && $("body.BEE").length==0 && 
        ...
        async function locationsHelper() {
                    ...
                    mutation.addedNodes.forEach(function (added_node) {
                        if (added_node.className == "autocomplete-suggestion-container") {
                            ...
                            if ($("#mBirthDate").length) {
                                currentBirthYearMatch = $("#mBirthDate").val().match(/[0-9]{3,4}/);
                            }
                            if ($("#mDeathDate").length) {
                                currentDeathYearMatch = $("#mDeathDate").val().match(/[0-9]{3,4}/);
                            }
            ...
            setTimeout(function () {
                observer2.observe($(".autocomplete-suggestions").eq(0)[0], { subtree: false, childList: true });
                if ($("#mDeathLocation").length) {
                    observer2.observe($(".autocomplete-suggestions").eq(1)[0], { subtree: false, childList: true });
                }
            }, 3000);
        }
        locationsHelper();
